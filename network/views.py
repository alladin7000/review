...
def profilepage(request,uname):
    try:
        ...
        userposts = None
    try:
        if Followingdata.objects.filter(username=uname,follower=request.user.username):
            is_follow = True
        else:
            is_follow = False
    except:
        is_follow = False
    try:
        followerlist = Followingdata.objects.filter(username=uname)
        followers = followerlist.count()
    except:
        followers = 0
    try:
        followinglist = Followingdata.objects.filter(follower=uname)
        following=followinglist.count()
    except:
        following = 0
    try:
        likeslist = Likes.objects.filter(likedby=request.user.username)
        likedids = []
        for l in likeslist:
            likedids.append(l.postid)
    except:
...
def followuser(request,name):
    try:
        flist = Followingdata.objects.get(username=name,follower=request.user.username)
    except:
        flist = Followingdata()
        flist.username = name
...
def followingposts(request):
    try:
        following = Followingdata.objects.filter(follower=request.user.username)
...
def followapi(request,name):
    try:
        if User.objects.get(username=name) :
            try:
                followerlist = Followingadata.objects.filter(username=name)
                followers = followerlist.count()
            except:
                followers = 0
            try:
                followinglist = Followingdata.objects.filter(follower=name)
                following=followinglist.count()
            except:
                following = 0
            try:
                if Followingdata.objects.filter(username=name,follower=request.user.username):
                    is_follow = True
                else:
                    is_follow = False
            except:
...
def likesapi(request,postid):
    if request.method == "GET":
        try:
            postlike = Likes.objects.get(postid=postid,likedby=request.user.username)
        ...
        if request.user.username == data.get("unlikedby"):
            likerow = Likes.objects.get(postid=data.get("id"),likedby=data.get("unlikedby"))
            likerow.delete()
            blikes = Blogpost.objects.get(id=data.get("id"))
            blikes.likes = data.get("likes")
            blikes.save()
